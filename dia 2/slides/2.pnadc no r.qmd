...
::::: columns
::: {.column width="20%"}
Fluxo de análise de dados[^2] que, geralmente, seguimos:
:::
...
[^2]: Fonte: Curso-R. Ciência de dados em R. Ver referências da aula.
...
-   O Google é uma ferramenta de busca capaz de selecionar alguns conteúdos associados à sua busca[^3].
:::

::: {.column width="50%"}
### Stack Overflow

-   É um site de pergunta e resposta muito utilizado em outros países. É fortemente alimentado por usuários de linguagem de programação (inclusive R!!!)[^4].
:::
:::::

[^3]: **Exemplo de busca de erro no Google:** quando recebe algum erro rodando um código em R, pode-se jogar a mensagem de erro no google.

[^4]: Para saber mais sobre boas práticas de construção de perguntas no Stack Overflow, ler pgs 44-45 de "Curso-R. Ciência de dados em R", na ementa do curso.
...
## [Peso amostral]{.theme-subtopic} {.smaller}
...
-   Realizar análises com a PNADC sem declarar/aplicar seus pesos incorre em um grave erro e pode gerar estimativas viesadas[^5].
...
[^5]: **Importância do SIDRA:** é exatamente para atenuar erros no cálculo de nossas estimativas que usamos o SIDRA para validar o modo como estamos declarando o peso.
...
-   **Pessoa:** todas de um domicílio recebem o mesmo peso[^6].
:::

[^6]: **Peso do domicílio:** embora todas as pessoas do domicílio recebam o mesmo peso, para análises domiciliares, devemos escolher somente uma unidade para representar o domicílio.
...
:::: {.column width="70%"}
::: nonincremental
**Coeficiente de Variação (CV) ou Desvio Padrão Relativo (DPR)**[^7]
...
[^7]: Referência IBGE: Quadro de referência para avaliação de estimativas, utilizado pelo IBGE, está disponível na pasta do curso.
...
:::: {.column width="70%"}
::: nonincremental
**Coeficiente de Variação (CV) ou Desvio Padrão Relativo (DPR)**[^8]
...
[^8]: Referência IBGE: Quadro de referência para avaliação de estimativas, utilizado pelo IBGE, está disponível na pasta do curso.
...
::: nonincremental
Primeiro, instalamos os pacotes em R[^9]...
...
[^9]: **Carregamento de pacotes:** Pacotes devem ser carregados todas as vezes que iniciamos uma sessão do R.

## [Pacotes para análise dos dados da PNADC em R]{.theme-subtopic} {.smaller}

::: nonincremental
Depois, carregamos os pacotes de interesse[^10]...
...
[^10]: **Instalação de pacotes:** Uma vez instalado o pacote, não é necessário fazer a sua instalação novamente.

## [Fazendo download dos bancos de dados em R]{.theme-subtopic} {.smaller}

**1. Diretamente no R**: É possível baixar diretamente no R, selecionando as variáveis desejadas e já contemplando o plano amostral[^11].

[^11]: **Tempo de download:** A depender da internet, o tempo para download dos dados pode variar bastante...
