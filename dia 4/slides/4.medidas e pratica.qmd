...
-   **Números absolutos** = `svytotal(~VARIAVEL)`

-   **Números relativos** (distribuição relativa; composição) = `svymean(~VARIAVEL)` 

-   **Proporção** (numerador está contido no denominador) = `svymean(~VARIAVEL)` 

-   **Taxa** (numerador **não** está contido no denominador) = `svyratio(~NUMERADOR, ~DENOMINADOR)`

-   **Razão** (divisão de duas grandezas quaisquer) = `svyratio(~NUMERADOR, ~DENOMINADOR)`

-   **Medidas de tendência central** (média, moda, mediana) = `svymean(~NUMERADOR, ~DENOMINADOR)`, `svyquantile(~VARIAVEL, quantiles=~c(.5))`
